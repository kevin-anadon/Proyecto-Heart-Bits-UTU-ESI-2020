...
checkdesinst=$(grep "INSTALL=0" $HOME/.bashrc)
if [ -z $checkdesinst ]
then 
sed -i "s/export INSTALL=1/export INSTALL=0/" $HOME/.bashrc
sed -i "/export CANTBACKUP=*/d" $HOME/.bashrc
else
#En el caso de que exista un install=0 por algun error
...
#Borro el comando que se halla en el archivo /etc/crontab
sed -i "/0 23 * /d" /etc/crontab
